...
class basic_unitary():
    ...
    """
    def __init__(self, expr, coeff = 0, re = 0., im = 0.):
        if isinstance(expr, str):
            self.expr = Pauli(expr)
            self.mat = self.expr.to_matrix()
            self.type = 'Pauli'
        elif isinstance(expr, Pauli):
            self.expr = expr
            self.mat = self.expr.to_matrix()
            self.type = 'Pauli'
            ...
            self.mat = expr
            self.type = 'Unitary'
        ## Suppose that the coeff is real and > 0, move negative sign to matrix
        self.coeff = coeff if coeff > 0 else - coeff
        if coeff < 0:
            self.mat = - self.mat
        if re != 0.: 
            self.real = re
    ...
    # def __repr__(self):
    #     if self.type == 'Pauli':
    #         return self.expr, str(self.coeff)
    #     else:
    #         return self.mat, str(self.coeff)
